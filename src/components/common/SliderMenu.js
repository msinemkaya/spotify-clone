...
export default function SliderMenu({ children }){
  ...
  return(
    <Container className={'relative py-2'}>
      {scrollX !== 0 && <SpotifyButton elevated type={'left'} className={'z-10 absolute top-2 left-0'}  />}
      ...
      {!scrolEnd && <SpotifyButton elevated type={'right'} className={'z-10 absolute top-2 right-0'}/>}
    </Container>
  );
}